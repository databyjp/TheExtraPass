...
def plot_hex_shot_chart(shots_df, teamname, period, stat_type,
                        ...
                        gridsize=None, min_samples=None, title=None, mode=None, colorscale_in=None):
    ...
    if stat_type == 'acc_abs':
        ...
        colorscale = 'RdYlBu_r'
        colorscale = ['DeepPink', '#ffffff', 'LightSkyBlue']
    ...
    if title is not None:
        ...
        title_txt = 'Shot chart<BR>'.upper() + title_suffix

    if colorscale_in is not None:
        colorscale = colorscale_in
...
def format_polar_chart(fig, marker_cmax, marker_cmin, mode="light"):
    ...
    fig.update_traces(marker=dict(line=dict(width=1.5, color=textcolor), symbol="circle"))
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            ...
            title_font={"size": 13, "color": textcolor},
        ),
        xaxis=dict(
            showgrid=True,
            gridcolor=gridcolor,
    ...
    fig.update_layout(coloraxis_colorbar=dict(
        ...
        tickvals=[marker_cmin, (marker_cmin + marker_cmax) / 2, marker_cmax],
        ticktext=ticktexts,
        titlefont=dict(size=12, color=textcolor),
        tickfont=dict(
            size=12,
            color=textcolor
        )
...
def plot_polar_pps(tmp_df, marker_cmin=65, marker_cmax=145, ref_size=5, color="pps", mode="light"):

    # marker avg should be about 105 for league avg pps

    labels = {
        "rel_pct": "Relative<BR>accuracy (%)", "pps": "PTS/100", "pts_pct": "% of Pts",
        "tbin": "SHOT ANGLE (degrees)", "rbin": "SHOT DISTANCE (ft.)",
        "freq_pct": "FREQ. (%)<BR>&nbsp;"
    }

    fig = px.scatter(tmp_df, x="tbin", y="rbin", size="freq_pct", color=color,
                     color_discrete_sequence=["Silver"],
                     color_continuous_scale=px.colors.diverging.RdYlBu_r, template="plotly_white",
                     range_x=[-95, 95], range_y=[-4, 40], range_color=[marker_cmin, marker_cmax],
                     height=550, width=720, size_max=24, labels=labels, hover_data=["attempts"]
                     )
    ...
    fig = format_polar_chart(fig, marker_cmax=marker_cmax, marker_cmin=marker_cmin, mode=mode)
